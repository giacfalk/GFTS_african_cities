...
ggplot(cities, aes(x=eFUA_name,  y=as.vector(area), fill=eFUA_name)) +
  theme_classic()+
  geom_col()+
...
ggsave(plot = last_plot(), device = "png", filename = "area_fua.png", scale=1.2)
...
ecdf <- ggplot(na.exclude(all), aes(x=layer.0,  weight = layer.2, group=ISO, colour=ISO)) +
  theme_classic()+
  stat_ecdf(size=0.75)+
  coord_cartesian(xlim=c(0.001, 60))+
  xlab('Minutes')+
  ylab("Cumulative fraction of the population")+
  scale_x_continuous(breaks = c(0, 5, 10, 20, 30, 60))+
  geom_vline(xintercept=c(5, 10, 20, 30, 60), linetype="dotted")+
  ggtitle("Walking time to the nearest (mapped) stop")+
  scale_y_continuous(labels = scales::percent_format(accuracy = 1))+
  scale_colour_discrete(name="City")

ggsave("ecdf_fua.png", ecdf, scale=0.85)
...
p_a <- ggplot(all_s, aes(x=ISO,  y=pwatt, fill=ISO)) +
  theme_classic()+
  geom_col()+
  ggtitle("Population weighted average travel time to the nearest (mapped) stop")+
  ylab("Minutes")+
  xlab("City")+
  theme(legend.position = "none")
...
all_prova <- all %>% group_by(ISO) %>% dplyr::summarise(pop5min=sum(layer.2[layer.0>15], na.rm = T), pop=sum(layer.2, na.rm = T))

p_b <- ggplot(all_prova, aes(x=ISO,  y=pop5min/pop, fill=ISO)) +
  theme_classic()+
  geom_col()+
  ggtitle("Share of population not within a 15-minute walk from the nearest (mapped) stop")+
  ylab("Population share (%)")+
  xlab("City")+
  theme(legend.position = "none")+
  scale_y_continuous(labels = scales::label_percent())

p_c <- ggplot(all_prova, aes(x=ISO,  y=pop5min/1e3, fill=ISO)) +
  theme_classic()+
  geom_col()+
  ggtitle("People not within a 15-minute walk from the nearest (mapped) stop")+
  ylab("Thousand people")+
  xlab("City")+
  theme(legend.position = "none")

ggsave(plot = cowplot::plot_grid(p_a, p_b, p_c, labels="AUTO", ncol = 1), device = "png", scale=1.5, filename = "deprivation_fua.png", height = 5.5*1.5/1.5)
